...
+\sum_{(x,y)\in \delta(v,r)} c(x,y)\cdot y_{x,y}\bigl(r-\dist_{N(v,r)}(v,x)\bigr).
\]

This quantity consists of 3 terms: a small constant term $\varepsilon\tau_X$ called \emph{seed value}, the contribution of edges inside the sphere, and the contribution of edges crossing the sphere boundary. We will choose $\epsilon=\frac{1}{n\ln n}$.
...
Since
\[
\operatorname{vol}_j(v_j,\tilde r)\le (1+\varepsilon)\cdot\tau_X,
\qquad
\operatorname{vol}_j(v_j,0)=\varepsilon\cdot\tau_X,
\]
we get
\[
c\bigl(\delta(U_j)\bigr)
\le
\frac{1}{\tilde r}\cdot\ln\left(\frac{1+\varepsilon}{\varepsilon}\right)\cdot
\operatorname{vol}_j(v_j,r_j).
\]

Summing over calls, edge-length contributions are at most $\tau_X$, and seed terms contribute at most $q\cdot\varepsilon\tau_X\le n\varepsilon\tau_X$. Hence
\[
\sum_{j=1}^q c\bigl(\delta(U_j)\bigr)
\le
\frac{1}{\tilde r}\cdot 
\ln\left(\frac{1+\varepsilon}{\varepsilon}\right)
\cdot \tau_X(1+\varepsilon n).
\]
